...
$sql = "select email from candidates where email=:email";
...
$stmt->execute();
    ...
    if($row['email']==$email){
		...
		header("location:insert_Candidates.php");
      
    }
    else{
		
        $sql = "INSERT into candidates(name,email,mobile,course,testimonial,studentid) values(:name,:email,:mobile,:course,:testimonial,:studentid)";
        $stmt = $pdo->prepare($sql);
