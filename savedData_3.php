...
$sql = "select email from users_3rd where email=:email";
...
//saves users data in database
	...
	if($row['email']==$email){
		...
		header("location:confirmation_3.php");
        

	}else{
		
        $sql = "INSERT into users_3rd(name,email,course,number,candidate) values(:name,:email,:course,:number,:candidate)";
