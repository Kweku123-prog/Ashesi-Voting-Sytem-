...
$sql = "select email from candidate_2nd where email=:email";
...
$stmt->execute();
    ...
    if($row['email']==$email){
		...
		header("location:insert_Candidate_2nd.php");
      
    }
    else{
		
        $sql = "INSERT into candidate_2nd(name,email,mobile,course,testimonial,studentid) values(:name,:email,:mobile,:course,:testimonial,:studentid)";
        $stmt = $pdo->prepare($sql);
